...
$('profileButton').onclick = () => { if (!account) showSelectedAvatar(selectedAvatarSeed, true); openModal(accountModal); };
...
const avatarSeeds = ['QuestKnight','MoonMage','DiceGoblin','DragonBard','ForestDruid','RuneRogue','CrystalCleric','StormPaladin','ShadowWarlock','SunElf','FrostRanger','MushroomMonk','StarSorcerer','IronBarbarian','CandleWizard','WildSatyr','RoyalBard','DeepDwarf'];
function showSelectedAvatar(seed, updateGlobalPreview = true) {
  selectedAvatarSeed = seed;
  const url = avatarUrl(seed);
  const preview = $('selectedAvatarPreview');
  if (preview) preview.src = url;
  const label = $('selectedAvatarName');
  if (label) label.textContent = seed.replace(/([a-z])([A-Z])/g, '$1 $2');
  $('avatarPicker')?.querySelectorAll('button').forEach(button => {
    const selected = button.dataset.seed === seed;
    button.classList.toggle('selected', selected);
    button.setAttribute('aria-pressed', String(selected));
  });
  if (updateGlobalPreview && !account) {
    $('topAvatar').src = url;
    $('composerAvatar').src = url;
  }
}

$('avatarPicker').innerHTML = avatarSeeds.map((seed, i) => `<button type="button" data-seed="${seed}" class="${i===0?'selected':''}" aria-label="Choose ${seed} avatar" aria-pressed="${i===0}"><img src="${avatarUrl(seed)}" alt="${seed} avatar"></button>`).join('');
showSelectedAvatar(selectedAvatarSeed, true);
$('avatarPicker').addEventListener('click', e => {
  const button = e.target.closest('button[data-seed]');
  if (!button) return;
  showSelectedAvatar(button.dataset.seed, true);
});
...
$('createAccount').onclick = async () => {
  ...
  showSelectedAvatar(selectedAvatarSeed, false);
